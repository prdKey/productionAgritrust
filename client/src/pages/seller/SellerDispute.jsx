...
import {
  ...
  ChevronDown, ChevronUp, Package, Clock, User, FileText,
} from "lucide-react";
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track disputes raised on your orders</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: "Refunded",       val: refunded,      color: "text-orange-600", icon: <XCircle className="w-4 h-4" /> },
        ].map(s => (
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-16 text-center border border-gray-200">
          ...
          <p className="text-gray-500 text-sm">None of your orders have active disputes.</p>
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.slice().sort((a, b) => b.id - a.id).map(order => {
            ...
            const isUnderReview = order.status === 7;
                  ...
                  <div className="bg-gradient-to-br from-red-50 via-orange-50 to-amber-50 p-5 lg:w-64 border-b lg:border-b-0 lg:border-r border-gray-200 flex-shrink-0">
                    <div className="space-y-3">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border flex items-center gap-1.5 w-fit ${
                        ...
                        {isRefunded ? "Buyer Refunded" : "Under Review"}
                      </span>
                      ...
                      <div className="pt-2 border-t border-gray-200 grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <p className={`font-bold text-sm ${isRefunded ? "text-gray-400 line-through" : "text-green-600"}`}>
                            {parseFloat(order.totalProductPrice).toFixed(2)} AGT
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Escrowed</p>
                          <p className="font-bold text-red-600 text-sm">{parseFloat(order.totalPrice).toFixed(2)} AGT</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 p-5 space-y-4">
                    ...
                    {isUnderReview ? (
                      <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-red-800">Dispute Under Admin Review</p>
                          <p className="text-xs text-red-600 mt-1">
                            A dispute has been raised for this order. The admin is reviewing the case. Payment of{" "}
                            <span className="font-bold">{parseFloat(order.totalProductPrice).toFixed(2)} AGT</span>{" "}
                            is held in escrow until resolved.
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-orange-800">Dispute Resolved — Buyer Refunded</p>
                          <p className="text-xs text-orange-600 mt-1">
                            The admin ruled in the buyer's favor. The full amount of{" "}
                            <span className="font-bold">{parseFloat(order.totalPrice).toFixed(2)} AGT</span>{" "}
                            was refunded to the buyer.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {dispute?.reason && (
                      <div>
                        <p className="text-xs text-gray-500 uppercase tracking-wide font-medium mb-1 flex items-center gap-1">
                          <FileText className="w-3.5 h-3.5" /> Dispute Reason
                          <span className="ml-1 text-gray-400 font-normal normal-case">
                            (filed by {dispute.openedByRole === "BUYER" ? "Buyer" : "Seller"})
                          </span>
                        </p>
                    ...
                    <div>
                      ...
                      <div className="bg-blue-50 rounded-xl p-3 border border-blue-200">
                        <p className="text-sm font-semibold text-gray-900">
                          {order.deliveryAddress?.name || order.buyerName || "Unknown"}
                        </p>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3 border border-gray-200 grid grid-cols-3 gap-3 text-sm">
                      <div>
                        <p className="text-xs text-gray-500 mb-0.5">You Earn</p>
                        <p className={`font-semibold ${isRefunded ? "text-gray-400 line-through" : "text-green-600"}`}>
                          {parseFloat(order.totalProductPrice).toFixed(2)} AGT
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 mb-0.5">Platform Fee</p>
                        <p className="font-semibold text-gray-700">{parseFloat(order.platformFee).toFixed(4)} AGT</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 mb-0.5">Logistics Fee</p>
                        <p className="font-semibold text-gray-700">{parseFloat(order.logisticsFee).toFixed(2)} AGT</p>
                      </div>
                    </div>
                        ...
                        <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                          ...
                          <div className="space-y-2 text-xs">
                            <div className="flex justify-between">
                              ...
                              <span className={`font-semibold ${isRefunded ? "text-gray-400 line-through" : "text-green-600"}`}>
                                {parseFloat(order.totalProductPrice).toFixed(2)} AGT
                              </span>
                            </div>
