// pages/seller/SellerNotifications.jsx
...
const ROLE = "SELLER"; // ← scope: seller panel lang
...
export default function SellerNotifications() {
  ...
  const fetchNotifications = async () => {
    try {
      setLoading(true);
      const data = await getNotifications(ROLE); // ← SELLER only
  ...
  const handleMarkAllAsRead = async () => {
    try { await markAllAsRead(ROLE); setNotifications(prev => prev.map(n => ({ ...n, read: true }))); } // ← SELLER only
    catch (err) { console.error(err); }
  };
  ...
  const handleDeleteAllRead = async () => {
    ...
    try { await deleteAllRead(ROLE); setNotifications(prev => prev.filter(n => !n.read)); setPage(1); } // ← SELLER only
    catch (err) { console.error(err); }
  };
            ...
            {paginated.map(n => {
              ...
              return (
                <div key={n.id} className={`bg-white rounded-lg shadow-sm border overflow-hidden transition-all ${
                  !n.read ? "border-l-4 border-l-blue-500 border-gray-200" : "border-gray-200 hover:shadow-md"
                }`}>
