// pages/buyer/BuyerNotifications.jsx
...
const ROLE = "BUYER"; // ← scope: buyer panel lang
...
export default function BuyerNotifications() {
  ...
  const fetchNotifications = async () => {
    try {
      setLoading(true);
      const data = await getNotifications(ROLE); // ← BUYER only
  ...
  const handleMarkAllAsRead = async () => {
    try {
      await markAllAsRead(ROLE); // ← BUYER only
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const newFiltered   = filteredNotifications.filter(n => n.id !== id);
  ...
  const handleDeleteAllRead = async () => {
    ...
    try {
      await deleteAllRead(ROLE); // ← BUYER only
            ...
            {paginated.map(n => {
              ...
              return (
                <div key={n.id} className={`bg-white rounded-lg shadow-sm border overflow-hidden transition-all ${
                  !n.read ? "border-l-4 border-l-green-500 border-gray-200" : "border-gray-200 hover:shadow-md"
                }`}>
