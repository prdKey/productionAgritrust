  ...
  export default function AdminDashboard() {
    ...
    // ── User breakdown — normalized role comparison ───────────────────────────
    const buyerCount     = users.filter(u => normalizeRole(u.role) === "buyer").length;
    const sellerCount    = users.filter(u => normalizeRole(u.role) === "seller").length;
    const logisticsCount = users.filter(u =>
      normalizeRole(u.role) === "logistics" || normalizeRole(u.role) === "logistic"
    ).length;

    const recentOrders = [...allOrders].sort((a, b) => b.id - a.id).slice(0, 5);

    // ── Status breakdown — includes status 10 as Cancelled ───────────────────
    const statusBreakdown = [
      ...
      { status: 10, label: "Cancelled (Auto)", color: "bg-gray-300"   },
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-red-700 mt-1">
                  {disputedOrders.length === 1
                    ? "There is 1 disputed order"
                    : `There are ${disputedOrders.length} disputed orders`
                  ...
                  <div className="flex items-center justify-between text-sm mt-2">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-semibold text-green-600">
                        {parseFloat(order.totalPrice || 0).toFixed(2)} AGT
                      </span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="divide-y divide-gray-100">
                ...
                {/* Fallback: show unmatched roles count */}
                {(() => {
                  const knownRoles = ["buyer", "seller", "logistics", "logistic"];
                  const otherCount = users.filter(u => !knownRoles.includes(normalizeRole(u.role))).length;
                  if (otherCount === 0) return null;
                  const pct = Math.round((otherCount / users.length) * 100);
                  return (
